...
class Sentinel1Scene:
    ...
    def __init__(self, scene_id, ard_type="OST_GTC", log_level=logging.INFO):
        ...
        # get metadata from scene identifier (first identifier row added by johanna)
        self.identifier = scene_id 
        ...
        elif self.mission_id == "S1C":
            self.orbit_offset = 172
            self.satellite = "Sentinel-1C"

        else:
            raise ValueError(f"Unsupported mission_id={self.mission_id} for scene {self.scene_id}")

        # own temp fix until self.rel_orbit=...

        if self.orbit_offset is None or self.abs_orbit is None:
            raise ValueError(
                f"Invalid orbit metadata: mission_id={self.mission_id}, "
                f"abs_orbit={self.abs_orbit}, identifier={self.identifier}"
            )
        base = ((int(self.abs_orbit) - int(self.orbit_offset)) % 175) + 1

        if self.mission_id == "S1C":
            # apply your custom shift here (but be careful: must still be 1..175)
            self.rel_orbit = ((base - 1 + 4) % 175) + 1   # +4 shift == your +5 vs +1 difference
        else:
            self.rel_orbit = base
